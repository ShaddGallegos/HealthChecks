...
## Title
keycloak HealthCheck Role

## Synopsis
The keycloak role performs product-specific health validation and reporting in the PostInstall_Healthcheck workflow.
...
Run this role through the corresponding top-level runner from /home/sgallego/GIT/HealthChecks:
    ...
    ansible-playbook run_postinstall.yml -e "products=['keycloak']"

Apply/remediation mode (opt-in):

    cd /home/sgallego/GIT/HealthChecks
    ansible-playbook run_postinstall.yml -e "products=['keycloak'] healthcheck_check_mode_only=false"
...
## Variables
Common role defaults discovered from PostInstall_Healthcheck/roles/keycloak/defaults/main.yml:
- keycloak_supported_rhel_major_versions
...
## Related Files
- PostInstall_Healthcheck/roles/keycloak/tasks/main.yml
- PostInstall_Healthcheck/roles/keycloak/defaults/main.yml
