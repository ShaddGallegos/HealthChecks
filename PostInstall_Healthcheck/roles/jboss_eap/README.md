...
## Title
jboss_eap HealthCheck Role

## Synopsis
The jboss_eap role performs product-specific health validation and reporting in the PostInstall_Healthcheck workflow.
...
Run this role through the corresponding top-level runner from /home/sgallego/GIT/HealthChecks:
    ...
    ansible-playbook run_postinstall.yml -e "products=['jboss_eap']"

Apply/remediation mode (opt-in):

    cd /home/sgallego/GIT/HealthChecks
    ansible-playbook run_postinstall.yml -e "products=['jboss_eap'] healthcheck_check_mode_only=false"
...
## Variables
Common role defaults discovered from PostInstall_Healthcheck/roles/jboss_eap/defaults/main.yml:
- jboss_eap_supported_rhel_major_versions
...
## Related Files
- PostInstall_Healthcheck/roles/jboss_eap/tasks/main.yml
- PostInstall_Healthcheck/roles/jboss_eap/defaults/main.yml
