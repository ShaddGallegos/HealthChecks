...
## Title
aap HealthCheck Role

## Synopsis
The aap role performs product-specific health validation and reporting in the PostInstall_Healthcheck workflow.
...
Run this role through the corresponding top-level runner from /home/sgallego/GIT/HealthChecks:
    ...
    ansible-playbook run_postinstall.yml -e "products=['aap']"

Apply/remediation mode (opt-in):

    cd /home/sgallego/GIT/HealthChecks
    ansible-playbook run_postinstall.yml -e "products=['aap'] healthcheck_check_mode_only=false"
...
## Variables
Common role defaults discovered from PostInstall_Healthcheck/roles/aap/defaults/main.yml:
- aap_product_repos
...
## Related Files
- PostInstall_Healthcheck/roles/aap/tasks/main.yml
- PostInstall_Healthcheck/roles/aap/defaults/main.yml
