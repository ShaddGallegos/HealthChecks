...
## Title
ceph HealthCheck Role

## Synopsis
The ceph role performs product-specific health validation and reporting in the PostInstall_Healthcheck workflow.
...
Run this role through the corresponding top-level runner from /home/sgallego/GIT/HealthChecks:
    ...
    ansible-playbook run_postinstall.yml -e "products=['ceph']"

Apply/remediation mode (opt-in):

    cd /home/sgallego/GIT/HealthChecks
    ansible-playbook run_postinstall.yml -e "products=['ceph'] healthcheck_check_mode_only=false"
...
## Variables
Common role defaults discovered from PostInstall_Healthcheck/roles/ceph/defaults/main.yml:
- ceph_product_repos
...
## Related Files
- PostInstall_Healthcheck/roles/ceph/tasks/main.yml
- PostInstall_Healthcheck/roles/ceph/defaults/main.yml
