...
## Title
data_grid HealthCheck Role

## Synopsis
The data_grid role performs product-specific health validation and reporting in the PostInstall_Healthcheck workflow.
...
Run this role through the corresponding top-level runner from /home/sgallego/GIT/HealthChecks:
    ...
    ansible-playbook run_postinstall.yml -e "products=['data_grid']"

Apply/remediation mode (opt-in):

    cd /home/sgallego/GIT/HealthChecks
    ansible-playbook run_postinstall.yml -e "products=['data_grid'] healthcheck_check_mode_only=false"
...
## Variables
Common role defaults discovered from PostInstall_Healthcheck/roles/data_grid/defaults/main.yml:
- data_grid_supported_rhel_major_versions
...
## Related Files
- PostInstall_Healthcheck/roles/data_grid/tasks/main.yml
- PostInstall_Healthcheck/roles/data_grid/defaults/main.yml
