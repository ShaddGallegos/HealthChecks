...
## Title
three_scale HealthCheck Role

## Synopsis
The three_scale role performs product-specific health validation and reporting in the PostInstall_Healthcheck workflow.
...
Run this role through the corresponding top-level runner from /home/sgallego/GIT/HealthChecks:
    ...
    ansible-playbook run_postinstall.yml -e "products=['three_scale']"

Apply/remediation mode (opt-in):

    cd /home/sgallego/GIT/HealthChecks
    ansible-playbook run_postinstall.yml -e "products=['three_scale'] healthcheck_check_mode_only=false"
...
## Variables
Common role defaults discovered from PostInstall_Healthcheck/roles/three_scale/defaults/main.yml:
- three_scale_supported_rhel_major_versions
...
## Related Files
- PostInstall_Healthcheck/roles/three_scale/tasks/main.yml
- PostInstall_Healthcheck/roles/three_scale/defaults/main.yml
