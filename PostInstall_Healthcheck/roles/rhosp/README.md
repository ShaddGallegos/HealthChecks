...
## Title
rhosp HealthCheck Role

## Synopsis
The rhosp role performs product-specific health validation and reporting in the PostInstall_Healthcheck workflow.
...
Run this role through the corresponding top-level runner from /home/sgallego/GIT/HealthChecks:
    ...
    ansible-playbook run_postinstall.yml -e "products=['rhosp']"

Apply/remediation mode (opt-in):

    cd /home/sgallego/GIT/HealthChecks
    ansible-playbook run_postinstall.yml -e "products=['rhosp'] healthcheck_check_mode_only=false"
...
## Variables
Common role defaults discovered from PostInstall_Healthcheck/roles/rhosp/defaults/main.yml:
- rhosp_product_repos
- rhosp_product_prereq_packages
- rhosp_product_required_secrets
- rhosp_product_required_mounts
...
## Related Files
- PostInstall_Healthcheck/roles/rhosp/tasks/main.yml
- PostInstall_Healthcheck/roles/rhosp/defaults/main.yml
