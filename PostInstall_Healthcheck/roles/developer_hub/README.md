...
## Title
developer_hub HealthCheck Role

## Synopsis
The developer_hub role performs product-specific health validation and reporting in the PostInstall_Healthcheck workflow.
...
Run this role through the corresponding top-level runner from /home/sgallego/GIT/HealthChecks:
    ...
    ansible-playbook run_postinstall.yml -e "products=['developer_hub']"

Apply/remediation mode (opt-in):

    cd /home/sgallego/GIT/HealthChecks
    ansible-playbook run_postinstall.yml -e "products=['developer_hub'] healthcheck_check_mode_only=false"
...
## Variables
Common role defaults discovered from PostInstall_Healthcheck/roles/developer_hub/defaults/main.yml:
- developer_hub_supported_rhel_major_versions
...
## Related Files
- PostInstall_Healthcheck/roles/developer_hub/tasks/main.yml
- PostInstall_Healthcheck/roles/developer_hub/defaults/main.yml
