...
## Title
quarkus HealthCheck Role

## Synopsis
The quarkus role performs product-specific health validation and reporting in the PostInstall_Healthcheck workflow.
...
Run this role through the corresponding top-level runner from /home/sgallego/GIT/HealthChecks:
    ...
    ansible-playbook run_postinstall.yml -e "products=['quarkus']"

Apply/remediation mode (opt-in):

    cd /home/sgallego/GIT/HealthChecks
    ansible-playbook run_postinstall.yml -e "products=['quarkus'] healthcheck_check_mode_only=false"
...
## Variables
Common role defaults discovered from PostInstall_Healthcheck/roles/quarkus/defaults/main.yml:
...
## Related Files
- PostInstall_Healthcheck/roles/quarkus/tasks/main.yml
- PostInstall_Healthcheck/roles/quarkus/defaults/main.yml
