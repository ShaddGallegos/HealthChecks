...
## Title
openshift HealthCheck Role

## Synopsis
The openshift role performs product-specific health validation and reporting in the PostInstall_Healthcheck workflow.
...
Run this role through the corresponding top-level runner from /home/sgallego/GIT/HealthChecks:
    ...
    ansible-playbook run_postinstall.yml -e "products=['openshift']"

Apply/remediation mode (opt-in):

    cd /home/sgallego/GIT/HealthChecks
    ansible-playbook run_postinstall.yml -e "products=['openshift'] healthcheck_check_mode_only=false"
...
## Variables
Common role defaults discovered from PostInstall_Healthcheck/roles/openshift/defaults/main.yml:
- openshift_product_repos
- openshift_product_prereq_packages
- openshift_product_required_secrets
- openshift_product_required_mounts
...
## Related Files
- PostInstall_Healthcheck/roles/openshift/tasks/main.yml
- PostInstall_Healthcheck/roles/openshift/defaults/main.yml
