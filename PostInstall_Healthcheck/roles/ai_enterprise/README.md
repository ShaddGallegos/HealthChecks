...
## Title
ai_enterprise HealthCheck Role

## Synopsis
The ai_enterprise role performs product-specific health validation and reporting in the PostInstall_Healthcheck workflow.
...
Run this role through the corresponding top-level runner from /home/sgallego/GIT/HealthChecks:
    ...
    ansible-playbook run_postinstall.yml -e "products=['ai_enterprise']"

Apply/remediation mode (opt-in):

    cd /home/sgallego/GIT/HealthChecks
    ansible-playbook run_postinstall.yml -e "products=['ai_enterprise'] healthcheck_check_mode_only=false"
...
## Variables
Common role defaults discovered from PostInstall_Healthcheck/roles/ai_enterprise/defaults/main.yml:
- ai_enterprise_supported_rhel_major_versions
...
## Related Files
- PostInstall_Healthcheck/roles/ai_enterprise/tasks/main.yml
- PostInstall_Healthcheck/roles/ai_enterprise/defaults/main.yml
