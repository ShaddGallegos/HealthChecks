...
## Title
amq HealthCheck Role

## Synopsis
The amq role performs product-specific health validation and reporting in the PostInstall_Healthcheck workflow.
...
Run this role through the corresponding top-level runner from /home/sgallego/GIT/HealthChecks:
    ...
    ansible-playbook run_postinstall.yml -e "products=['amq']"

Apply/remediation mode (opt-in):

    cd /home/sgallego/GIT/HealthChecks
    ansible-playbook run_postinstall.yml -e "products=['amq'] healthcheck_check_mode_only=false"
...
## Variables
Common role defaults discovered from PostInstall_Healthcheck/roles/amq/defaults/main.yml:
- amq_supported_rhel_major_versions
...
## Related Files
- PostInstall_Healthcheck/roles/amq/tasks/main.yml
- PostInstall_Healthcheck/roles/amq/defaults/main.yml
