...
## Title
acs HealthCheck Role

## Synopsis
The acs role performs product-specific health validation and reporting in the PostInstall_Healthcheck workflow.
...
Run this role through the corresponding top-level runner from /home/sgallego/GIT/HealthChecks:
    ...
    ansible-playbook run_postinstall.yml -e "products=['acs']"

Apply/remediation mode (opt-in):

    cd /home/sgallego/GIT/HealthChecks
    ansible-playbook run_postinstall.yml -e "products=['acs'] healthcheck_check_mode_only=false"
...
## Variables
Common role defaults discovered from PostInstall_Healthcheck/roles/acs/defaults/main.yml:
- acs_supported_rhel_major_versions
...
## Related Files
- PostInstall_Healthcheck/roles/acs/tasks/main.yml
- PostInstall_Healthcheck/roles/acs/defaults/main.yml
