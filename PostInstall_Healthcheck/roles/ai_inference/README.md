...
## Title
ai_inference HealthCheck Role

## Synopsis
The ai_inference role performs product-specific health validation and reporting in the PostInstall_Healthcheck workflow.
...
Run this role through the corresponding top-level runner from /home/sgallego/GIT/HealthChecks:
    ...
    ansible-playbook run_postinstall.yml -e "products=['ai_inference']"

Apply/remediation mode (opt-in):

    cd /home/sgallego/GIT/HealthChecks
    ansible-playbook run_postinstall.yml -e "products=['ai_inference'] healthcheck_check_mode_only=false"
...
## Variables
Common role defaults discovered from PostInstall_Healthcheck/roles/ai_inference/defaults/main.yml:
- ai_inference_supported_rhel_major_versions
...
## Related Files
- PostInstall_Healthcheck/roles/ai_inference/tasks/main.yml
- PostInstall_Healthcheck/roles/ai_inference/defaults/main.yml
